...
/*   Updated: 2024/08/31 14:02:51 by akyoshid         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(void)
{
	...
	// Linux
	printf_count += printf("%k%r%v%w%y% %B%D%H%J%K%M%N%O%P%Q%R%T%U%V%W%Y\n");
	ft_printf_count += ft_printf("%k%r%v%w%y% %B%D%H%J%K%M%N%O%P%Q%R%T%U%V%W%Y\n");
	ft_check_error(printf_count, ft_printf_count);

	printf("\n=== If undefined conversion specifications & %%%% are included in fmt, can it correctly take va? ===\n");
	printf_count += printf("| %c | %s | %% | %d | %i | %u | %w | %x | %X | %p |\n", c, str, INT_MIN, INT_MIN + 1, UINT_MAX, UINT_MAX, UINT_MAX - 1, str);
	ft_printf_count += ft_printf("| %c | %s | %% | %d | %i | %u | %w | %x | %X | %p |\n", c, str, INT_MIN, INT_MIN + 1, UINT_MAX, UINT_MAX, UINT_MAX - 1, str);
	ft_check_error(printf_count, ft_printf_count);

	printf("\n=== More VAs than conversion specifications ===\n");
	printf_count += printf("| %c | %s | %% | %d | %i | %w | %u |\n", c, str, INT_MIN, INT_MAX, UINT_MAX, 0, 12345, str);
	ft_printf_count += ft_printf("| %c | %s | %% | %d | %i | %w | %u |\n", c, str, INT_MIN, INT_MAX, UINT_MAX, 0, 12345, str);
	ft_check_error(printf_count, ft_printf_count);
